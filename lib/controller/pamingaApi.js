...
module.exports = {
    ...
    'AddContacts': (req, res) => {
        ...
        if (contacts && phone) {
            ...
            connection.query(query, (error, data1, fields) => {
                if (error) {
                    resHndlr.apiResponder(req, res, "error to get data ..!!!!", ERROR, ERROR_False);
                } else if (data1.length > 0) {

                    if (data1[0].phone == phone) {

                        console.log("contacts.lengthcontacts.length", contacts)
                        ...
                        for (let i = 0; i < contacts.length; i++) {
                            let query = 'select * from users WHERE phone ="' + contacts[i] + '"';
                            ...
                            connection.query(query, (error, data, fields) => {
                                if (error) {
                                    ...
                                    myfun(data[0].id, contacts.length, i)
                        ...
                        //////////////////
                        var mobData = [];
                        function myfun(id, count, i) {
                            mobData.push(id);
                            console.log("mobDate----------", mobData)
                            if ((i + 1) == count) {
                                console.log("there are the reqParam -----javed----", mobData.toString())
                                var reqParam = mobData.toString();
                                let query = 'select * from mycontacts WHERE uid ="' + req.user.id + '"';

                                connection.query(query, (error, data, fields) => {
                                    if (error) {
                                        resHndlr.apiResponder(req, res, "error to get data ..!!!!", ERROR, ERROR_False);
                                    } else if (data.length > 0) {
                                        var query3 = 'UPDATE mycontacts SET  contacts = "' + reqParam + '"WHERE uid ="' + req.user.id + '"';
                                        connection.query(query3, function (error, results, fields) {
                                            if (error) {
                                                console.log("there are the error", error);
                                                resHndlr.apiResponder(req, res, "there are the server error!!", ERROR, ERROR_False);

                                            } else {
                                                //console.log("there are the results successfully submit", results)
                                                resHndlr.apiResponder(req, res, "Query updated successfully", SUCCESS, ERROR_True, results[0]);
                                        ...
                                        var query3 = 'insert into mycontacts(uid,contacts)values("' + id + '","' + reqParam + '")'
                                        connection.query(query3, function (error, results, fields) {
                                            ...
                                            if (error) {
                                                ...
                                                //console.log("there are the results successfully submit", results)
                                                resHndlr.apiResponder(req, res, "Query submited successfully", SUCCESS, ERROR_True, results[0]);
                        ...
                        resHndlr.apiResponder(req, res, "phone no not exist", ERROR, ERROR_False);

                    }
                }
            })
        } else {
            resHndlr.apiResponder(req, res, "please enter phone and contacts", ERROR, ERROR_False);
        }
    },
    ...
    // },
    'getMyStatus': (req, res) => {
        console.log("there are the data -------------", req.user.id)
