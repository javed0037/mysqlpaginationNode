...
module.exports = {
  ...
  'getallUser': (req, res) => {
    var query1 = 'SELECT COUNT(*) AS count FROM users where status =1';
    connection.query(query1, function (error, results, fields) {
      if (error) {
        console.log("there are the error", error);
        res.json({ message: "some things wents wrongs", status: 400 })
      } else if (results.length > 0) {
        var query2 = "SELECT COUNT(*) AS count FROM users where status = 0";
        connection.query(query2, function (error, results1, fields) {
          if (error) {
            ...
            var resultObj = {
              ...
              inactive: results1[0].count

            }
            console.log('The solution is: ', resultObj);
            return res.json({ message: "user get successfully", data: resultObj })
            console.log('The solution is: ', resultObj);
          }
        })
      }
